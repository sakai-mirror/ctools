...
extends PagedResourceActionII
{
	...
	{		
		...
		if (attachment_mode != null)
		{
			...
			{
				template = AttachmentAction.buildHelperContext (portlet, context, data, state);
				return template;
			}
...
//			o.setDeliveryId(clientWindowId(state, portlet.getID()));
	...
	{	
		...
		String contextString = (String) state.getAttribute (STATE_CONTEXT_STRING);
		context.put("accessPointUrl", ServerConfigurationService.getAccessUrl() + AssignmentService.gradesSpreadsheetReference(contextString, null));
		...
		if (state.getAttribute(STATE_MESSAGE) == null)
		{
			try
			{
				...
				else
				{
					...
					try
					{			
						AssignmentSubmissionEdit edit = AssignmentService.addSubmission ((String) state.getAttribute (STATE_CONTEXT_STRING), assignmentId);
						edit.setSubmittedText (text);
						edit.setHonorPledgeFlag (Boolean.valueOf (honorPledgeYes).booleanValue ());
						edit.setSubmitted (false);
						//edit.addSubmitter (u);
					...
					try
					{
						...
						if (sEdit.getReturned())
						{
							...
							if (sEdit.getGraded())
							{
								...
								String previousGrades = (String) sEdit.getProperties().getProperty(ResourceProperties.PROP_SUBMISSION_SCALED_PREVIOUS_GRADES);
								if (previousGrades == null)
								{
									previousGrades = (String) sEdit.getProperties().getProperty(ResourceProperties.PROP_SUBMISSION_PREVIOUS_GRADES);
									if (previousGrades != null)
									{		
										int typeOfGrade = a.getContent ().getTypeOfGrade ();
										if (typeOfGrade==3)
	...
	protected void setNewAssignmentParameters(RunData data, boolean validify)
	{
		...
		if (params.getString (ResourceProperties.NEW_ASSIGNMENT_CHECK_ADD_DUE_DATE)!=null && params.getString (ResourceProperties.NEW_ASSIGNMENT_CHECK_ADD_DUE_DATE).equalsIgnoreCase (Boolean.TRUE.toString ()))
		{
		...
		if (params.getString (ResourceProperties.NEW_ASSIGNMENT_CHECK_AUTO_ANNOUNCE)!=null && params.getString (ResourceProperties.NEW_ASSIGNMENT_CHECK_AUTO_ANNOUNCE).equalsIgnoreCase (Boolean.TRUE.toString ()))
		{
		...
		{		
			...
			if (state.getAttribute(STATE_MESSAGE) == null)
			{
				...
				try
				{
					...
					{
						// add to schedule and announcement is not successful
	...
	*/
	protected void initState(SessionState state, VelocityPortlet portlet, JetspeedRunData data)
	{
		...
		if (cService == null)
		{
			cService = org.sakaiproject.service.legacy.calendar.cover.CalendarService.getInstance();
			...
			{
				calendarId = cService.
						calendarReference(ToolManager.getCurrentPlacement().getContext(), SiteService.MAIN_CONTAINER);
				try
				{
		...
		if (aService == null)
		{
			aService = org.sakaiproject.service.legacy.announcement.cover.AnnouncementService.getInstance();
			...
			{
				channelId = aService.
						channelReference(ToolManager.getCurrentPlacement().getContext(), SiteService.MAIN_CONTAINER);
				try
				{
...
//		}
	 ...
	 */
	private class AssignmentComparator
	implements Comparator
	{
		...
		public int compare ( Object o1, Object o2)
		{
			...
			else if (m_criteria.equals (SORTED_BY_NUM_UNGRADED))
			{
				...
				{
					AssignmentSubmission submission1 = (AssignmentSubmission) submissions1.next ();
					if (submission1.getSubmitted() && !submission1.getGraded())
						ungraded1++;
				}
				...
				{
					AssignmentSubmission submission2 = (AssignmentSubmission) submissions2.next ();
					if (submission2.getSubmitted() && !submission2.getGraded())
						ungraded2++;
				}
			...
			else if (m_criteria.equals (SORTED_SUBMISSION_BY_GRADE))
			{
				...
				// if scale is points
				if ((((AssignmentSubmission) o1).getAssignment().getContent().getTypeOfGrade() == 3) && ((((AssignmentSubmission) o2).getAssignment().getContent().getTypeOfGrade() == 3)))
				{
					if (grade1.equals(""))
					{
						result = -1;
			...
			if (submission !=null)
				if (submission.getSubmitted ())
					...
					else
					{
						...
						if (submission.getTimeSubmitted ().after (assignment.getDueTime ()))
							status = status + rb.getString("late");
					}
				else
	...
	*/
					...
					try
					{
						...
						for (int k=0; k<assignmentSubmissions.size(); k++)
						{
							...
							if (search != null && !search.equals(""))
							{
								...
								{
									if (s!=null && (s.getSubmitted() || (s.getReturned() && (s.getTimeLastModified().before(s.getTimeReturned())))))
									{
	 ...
	 * @param checkForFormattingErrors Whether to check for formatted text errors - if true, look
	 *   for errors in the formatted text.  If false, accept the formatted text without looking for errors.
	...
	/**
	 * Processes the given assignmnent feedback text, as returned
	 * from the user's browser.  Makes sure that the Chef-style
	 * markup {{like this}} is properly balanced.
	 */
	...
	/** 
	 * Called to deal with old Chef-style assignment feedback annotation,
	 * {{like this}}.  
	...
	/**
	 * Escapes the given assignment feedback text, to be edited
	 * as formatted text (perhaps using the formatted text widget)
	 */
	...
	*/
	...
	private boolean alertGlobalNavigation(SessionState state)
	{
		...
		if (mode.equals (MODE_STUDENT_VIEW_SUBMISSION)
			...
			|| mode.equals (MODE_INSTRUCTOR_PREVIEW_ASSIGNMENT)
